...
namespace AdventOfCode.Problems.Day3
{
    ...
    class Part1
	{
        ...
        private static IEnumerable<Point> FindGridPoints(string wire)
        {
            var currentPoint = new Point(0, 0);

            foreach(var vector in wire.Split(','))
            {
                ...
                {
                    case 'R':
                        for (int i = 1; i <= length; i++) yield return new Point(currentPoint.X + i, currentPoint.Y);
                    ...
                    case 'L':
                        ...
                        break;
                    case 'U':
                        for (int i = 1; i <= length; i++) yield return new Point(currentPoint.X, currentPoint.Y + i);
        ...
        public static Object Solve()
        {
            ...
            Parallel.ForEach(gridPointsToCheckWire1, new ParallelOptions { MaxDegreeOfParallelism = 8 }, point1 => {
                foreach (var point2 in gridPointsToCheckWire2)
                {
                    if (point1.Equals(point2))
                    {
                        var distance = 2 + gridPointsForWire1.TakeWhile(point => !point.Equals(point1)).Count() + gridPointsForWire2.TakeWhile(point => !point.Equals(point1)).Count();
                        lock (_lock)
                        {
                            shortestDistance = Math.Min(shortestDistance, distance);
                        }
