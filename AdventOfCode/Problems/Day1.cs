...
namespace AdventOfCode.Problems.Day1
{
    ...
    class Part2
    {
        ...
        public static Object Solve()
        {
            return massValues.Select(CalculateFuelForMass).Sum();
        }

        public static int CalculateFuelForMass(int mass)
        {
            int neededFuel = ((int)Math.Floor(mass / 3.0d)) - 2;
            if (neededFuel > 0)
            {
                neededFuel += CalculateFuelForMass(neededFuel);
            }
            return Math.Max(0, neededFuel);
        }
    }
}